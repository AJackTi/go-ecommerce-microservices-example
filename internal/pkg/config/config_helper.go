...
import (
	...
	typeMapper "github.com/AJackTi/go-ecommerce-microservices-example/internal/pkg/reflection/typemapper"
)
...
func BindConfigKey[T any](configKey string, environments ...environment.Environment) (T, error) {
	...
	viper.SetConfigFile(fmt.Sprintf("config.%s", currentEnv))
...
// Returns:
//
// string: The path of the directory containing of the config file, or an empty string if not found.
...
func searchForConfigFileDir(rootDir string, env environment.Environment) (string, error) {
	...
	err := filepath.Walk(rootDir, func(path string, info fs.FileInfo, err error) error {
		...
		if !info.IsDir() && strings.EqualFold(info.Name(), fmt.Sprintf("config.%s.json", env)) || strings.EqualFold(info.Name(), fmt.Sprintf("config.%s.yaml", env)) || strings.EqualFold(info.Name(), fmt.Sprintf("config.%s.yml", env)) {
